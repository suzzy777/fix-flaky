...
def _collect_reproflake_docker_coverage_files(test_input: TestInput) -> list[str]:
    ...
    if module and module != ".":
        cd_part = f"cd /app/source/{module} && "
    else:
        cd_part = "cd /app/source && "

    # Copy ReproFlake coverage tools into the extracted Flaky source.
    for item in [
        "jacocoagent.jar",
        "jacococli.jar",
        "coverage_generator.sh",
        "modify_pom_for_coverage.sh",
        "python-scripts",
    ]:
        src = workdir / item
        dst = source_root / item

        if not src.exists():
            logger.warning("Coverage helper missing: %s", src)
            return []
    ...
    if not full_test_name:
        full_test_name = f"{test_class}#{test_input.test_func}"
    
    mvn_cmd = (
        "cd /app/source && "
        "chmod +x coverage_generator.sh modify_pom_for_coverage.sh && "
        f"./coverage_generator.sh {shlex.quote(module)} . {shlex.quote(full_test_name)} 1"
    )
    cmd = [
        "docker", "run", "--rm",
    ...
    report_paths = _find_jacoco_reports(source_root, module)
    if not report_paths:
        logger.warning("JaCoCo report not found under Docker coverage source: %s", source_root)

    # artifact_files: list[str] = []
    # seen_artifact_files: set[str] = set()
    # for report_path in report_paths:
    #     for file in parse_jacoco_xml(str(report_path), str(source_root)):
    #         if file not in seen_artifact_files:
    #             seen_artifact_files.add(file)
    #             artifact_files.append(file)
    artifact_files: list[str] = []
    seen_artifact_files: set[str] = set()

    parse_root = source_root
    if module and module != ".":
        parse_root = source_root / module
    ...
    # Avoid leaving root-owned files in data/<issue>_coverage_ctx after Docker writes target/.m2 files.
    _docker_chown(source_root.parent)
